...
class Detail(SmartView):
    restrict_access = False

    template = "messages/detail.mak"

    # For Mobile vs Desktop
    mak_desktop = "messages/detail.mak"
    mak_mobile = "messages/m_detail.mak"
    ...
    def is_mobile(cls, request):
        ...
        if 'HTTP_USER_AGENT' in request.headers.environ:
            ...
            b = Detail.reg_b.search(user_agent)
            v = Detail.reg_v.search(user_agent[0:4])
            if b or v:
                return True

        return False

    @view_config(route_name="inbox:id", request_method="GET")
    @view_config(route_name="intro_inbox:id", request_method="GET")
    def inbox_details(self):
        ...
        message = self.messaging.get(id)
        message = message.deflate()
        dic['member'] = member
        ...
        dic['message']['view'] = 'inbox'

        return self.render(dic)

    @view_config(route_name="outbox:id", request_method="GET")
    @view_config(route_name="intro_outbox:id", request_method="GET")
    def outbox_details(self):
        id = int(self.request.matchdict["message_id"])
        id = int(id)
